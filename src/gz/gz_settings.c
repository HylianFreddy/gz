...
static int input_pressrel_proc(struct menu_item *item,
                               enum menu_callback_reason reason,
                               void *data)
{
  if (reason == MENU_CALLBACK_SWITCH_ON)
...
struct menu *gz_settings_menu(void)
{
  ...
  menu_add_option(&menu, 20, 8, "frames\0""seconds\0", lag_unit_proc, NULL);
  menu_add_static(&menu, 0, 9, "timer", 0xC0C0C0);
  menu_add_checkbox(&menu, 16, 9, timer_proc, NULL);
  menu_add_positioning(&menu, 18, 9,
  ...
  menu_add_static(&menu, 0, 11, "pointer watches", 0xC0C0C0);
  menu_add_checkbox(&menu, 16, 11, pointer_watches_proc, NULL);
  /* behavior controls */
  menu_add_static(&menu, 0, 12, "macro input", 0xC0C0C0);
  menu_add_checkbox(&menu, 16, 12, macro_input_proc, NULL);
  menu_add_static(&menu, 0, 13, "break type", 0xC0C0C0);
  menu_add_option(&menu, 16, 13, "normal\0""aggressive\0",
                  break_type_proc, NULL);
  menu_add_submenu(&menu, 0, 14, &commands, "commands");
  /* settings commands */
  menu_add_button(&menu, 0, 15, "save settings", save_settings_proc, NULL);
  menu_add_button(&menu, 0, 16, "load settings", load_settings_proc, NULL);
  menu_add_button(&menu, 0, 17, "restore defaults",
